...
exports.getItems = (cb) => {
  db.query(
    `SELECT items.id, items.name, items.price, items.created_at, items.updated_at FROM items `,
    cb
  );
};
...
exports.getItemsSort = (sort, column = item.name, cb) => {
  db.query(
    `SELECT items.id, items.name, items.price, category.name AS category_name, items.created_at, items.updated_at FROM items LEFT JOIN category ON items.category_id = category.id ORDER BY ${column} ${sort}`,
    cb
  );
};
...
exports.getItemByCondNSort = (cond, sort, column = item.name, cb) => {
  db.query(
    `SELECT items.id, items.name, items.price FROM items WHERE items.name LIKE '%${cond}%' ORDER BY ${column} ${sort}`,
    cb
  );
};
